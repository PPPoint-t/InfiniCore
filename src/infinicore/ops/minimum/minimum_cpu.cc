...
namespace infinicore::op::minimum_impl::cpu {

template <typename T, typename CompT>
inline T min_op(T a, T b) {
    CompT val_a = utils::cast<CompT>(a);
    CompT val_b = utils::cast<CompT>(b);
    return utils::cast<T>(std::min(val_a, val_b));
}

template <typename T, typename CompT>
void minimum_kernel(const T *input_ptr, const T *other_ptr, T *output_ptr, size_t numel) {
#pragma omp parallel for
    for (size_t i = 0; i < numel; ++i) {
        output_ptr[i] = min_op<T, CompT>(input_ptr[i], other_ptr[i]);
    }
}

template <typename T, typename CompT>
void minimum_strided_kernel(const T *in_data, const T *other_data, T *out_data,
...
#pragma omp parallel for
    for (size_t i = 0; i < numel; ++i) {
        ...
        out_data[out_offset] = min_op<T, CompT>(in_data[in_offset], other_data[other_offset]);
    }
}

void calculate_minimum(Tensor input, Tensor other, Tensor output) {
    auto dtype = input->dtype();
    if (other->dtype() != dtype || output->dtype() != dtype) {
        throw std::runtime_error("Dtype mismatch in minimum op");
    }
