...
typedef struct TanhOp {
  ...
  __device__ __forceinline__ T operator()(const T &input) const {
    auto tanh_f32 = [] __device__ (float x) {
      double xd = static_cast<double>(x);
      double yd = std::tanh(xd);
    ...
    if constexpr (std::is_same_v<T, half2>) {
        ...
        return __floats2bfloat162_rz(tanh_f32(f0), tanh_f32(f1));
    } else if constexpr (std::is_same_v<T, cuda_bfloat16>) {
        float xf = __bfloat162float(input);
        return __float2bfloat16_rz(tanh_f32(xf));
    } else if constexpr (std::is_same_v<T, float>) {
        return tanh_f32(input);
